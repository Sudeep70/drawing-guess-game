...
module.exports = (io, socket) => {
  ...
  socket.on('room:create', ({ playerName, difficulty } = {}) => {
    ...
    const room = createRoom(roomCode, socket.id, playerName.trim(), level);
    ...
    logger.info(`[${roomCode}] Created by ${playerName} (Difficulty: ${level})`);
  });
  ...
  socket.on('player:reconnect', ({ roomCode, oldSocketId } = {}) => {
    ...
    if (room.round.drawerSocketId === oldSocketId) {
      room.round.drawerSocketId = socket.id;
    }

    const idx = room.round.drawerOrder.indexOf(oldSocketId);
    if (idx !== -1) {
      room.round.drawerOrder[idx] = socket.id;
    }
    ...
    socket.to(code).emit('room:playerJoined', {
      player: playerSnapshot(room.players[socket.id])
    });
  ...
  function handleLeave(isDisconnect) {
    ...
    reconnectTimers[socket.id] = setTimeout(() => {
      ...
      logger.info(`[${roomCode}] ${player.name} grace expired — removed`);
  ...
  function checkRoomEmpty(roomCode) {
